<?php
/**
 * Class JvRegistrationModel
 *
 * Everything registration-related happens here.
 */
class JvRegistrationModel
{
    public static function getJvValue ()
    {
        $database = DatabaseFactory::getFactory()->getConnection();

        $sql = "SELECT jv FROM users WHERE user_id = :user_id";
    ...
    public static function registerNewUser()
    {
        ...
        $validation_result = JvRegistrationModel::registrationInputValidation(Request::post('captcha'), $user_name, $user_password_new, $user_password_repeat, $user_email);
        ...
        if (!JvRegistrationModel::writeNewUserToDatabase($user_name, $user_password_hash, $user_email, time(), $user_activation_hash, $jv)) {
        ...
        if (JvRegistrationModel::sendVerificationEmail($user_id, $user_email, $user_activation_hash)) {
            Session::add('feedback_positive', Text::get('FEEDBACK_ACCOUNT_SUCCESSFULLY_CREATED'));
            return true;
        }
        // if verification email sending failed: instantly delete the user
        JvRegistrationModel::rollbackRegistrationByUserId($user_id);
     ...
     * @return bool
     */
    public static function writeNewUserToDatabase($user_name, $user_password_hash, $user_email, $user_creation_timestamp, $user_activation_hash, $jv)
    {
        $database = DatabaseFactory::getFactory()->getConnection();
        // write new users data into database
        $sql = "INSERT INTO users (user_name, user_password_hash, user_email, user_creation_timestamp, user_activation_hash, user_provider_type, jv)
                    VALUES (:user_name, :user_password_hash, :user_email, :user_creation_timestamp, :user_activation_hash, :user_provider_type, :jv)";
        $query = $database->prepare($sql);
